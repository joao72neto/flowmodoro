...
```
*(Não esqueça de adicionar mapeamento para `updatedAt` no `SessionMapper` se for retornar no DTO, e no DTO)*
...
const logout = () => {
    showWarning({
        ...
        message: "Ao sair, todos os dados que ainda não foram sincronizados com o servidor serão perdidos. Certifique-se de realizar o backup ou esperar a sincronização.",
...
3. **Limpeza do Banco (Wipe)**: Fazer logout e inspecionar o DevTools (Application > IndexedDB). Todas as *object stores* devem estar limpas. Logar em conta nova não deve vazar dados da anterior.
