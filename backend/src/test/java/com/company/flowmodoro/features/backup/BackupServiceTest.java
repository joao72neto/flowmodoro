...
class BackupServiceTest {
    ...
    @Test
    @DisplayName(
        "Should throw USER_MISMATCH exception when backup userId does not match auth userId"
    )
    ...
    @Test
    @DisplayName(
        "Should throw ORPHAN_ENTITIES exception when a tag references a nonexistent project"
    )
    void shouldThrowExceptionWhenTagHasOrphanProject() {
        ...
        when(projectRepository.findByUserId(userId)).thenReturn(
            Collections.emptyList()
        );
        when(tagRepository.findByUserId(userId)).thenReturn(
            Collections.emptyList()
        );
        ...
        assertThat(ex.getErrors()).anyMatch(err ->
            err.contains(orphanProjectId.toString())
        );
    }

    @Test
    @DisplayName(
        "Should throw ORPHAN_ENTITIES exception when a session references a nonexistent tag"
    )
    void shouldThrowExceptionWhenSessionHasOrphanTag() {
        ...
        assertThat(ex.getCode()).isEqualTo(BackupErrorCode.ORPHAN_ENTITIES);
        assertThat(ex.getErrors()).anyMatch(err ->
            err.contains(orphanTagId.toString())
        );
    }

    @Test
    @DisplayName(
        "Should successfully import backup with new projects, tags, and sessions"
    )
    void shouldSuccessfullyImportValidBackup() {
        ...
        when(projectRepository.findByIdAndUserId(projectId, userId)).thenReturn(
            Optional.empty()
        );
        when(tagRepository.findByIdAndUserId(tagId, userId)).thenReturn(
            Optional.empty()
        );
        when(sessionRepository.findByIdAndUserId(sessionId, userId)).thenReturn(
            Optional.empty()
        );

        ProjectModel savedProject = ProjectModel.builder()
            .id(projectId)
            .name("Flowmodoro")
            .build();
        TagModel savedTag = TagModel.builder()
            .id(tagId)
            .name("Development")
            .build();
        when(projectRepository.findById(projectId)).thenReturn(
            Optional.of(savedProject)
        );
    ...
    @Test
    @DisplayName(
        "Should upsert existing project when imported version is newer"
    )
    void shouldUpsertExistingProjectWhenNewer() {
        ...
        ArgumentCaptor<ProjectModel> captor = ArgumentCaptor.forClass(
            ProjectModel.class
        );
