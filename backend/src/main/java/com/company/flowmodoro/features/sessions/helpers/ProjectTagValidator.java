...
public class ProjectTagValidator {
	...
	public void validateProjectAndTag(SessionModel session, String userId) {
		if (session.getProject() != null && session.getProject().getId() != null) {
			ProjectModel project = projectService.findById(session.getProject().getId(), userId);
			session.setProject(project);

			if (session.getTag() != null && session.getTag().getId() != null) {
				TagModel tag = project.getTags()
					.stream()
					.filter(t -> t.getId().equals(session.getTag().getId()))
					.findFirst()
					.orElseThrow(() -> new InvalidSessionException(SessionErrorCode.INVALID_SESSION,
							"Tag does not belong to the selected project"));
				session.setTag(tag);
			}
		}
		else if (session.getTag() != null && session.getTag().getId() != null) {
			throw new InvalidSessionException(SessionErrorCode.INVALID_SESSION,
