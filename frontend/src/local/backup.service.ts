...
class BackupService {
  async importData(file: File): Promise<void> {
    ...
    await db.transaction("rw", db.projects, db.tags, db.sessions, async () => {
      await db.projects.bulkPut(projectMapper.fromPayloadList(data.projects));
      await db.tags.bulkPut(tagMapper.fromPayloadList(data.tags));
      await db.sessions.bulkPut(sessionMapper.fromPayloadList(data.sessions));
    });
  }
